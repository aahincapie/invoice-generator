...
1. **Saved clients + bill-to auto-fill (with per-client invoice codes)** — a client directory; picking a client fills the invoice address block.
...
Invoice numbers follow **`{user prefix}-{client code}-{year}-{sequence}`** — e.g. `AHI-EML-2026-014`. The user prefix (`AHI`) is set once in Settings; each client carries its own 3-letter code; the year and zero-padded sequence complete it. Changing the client on an invoice regenerates the number.

The company block printed in the invoice header (trading name, legal address, postal code, phone, email, website) and the payee name in the payment-details block are **driven by the Settings profile** — no hard-coded company text anywhere in the invoice sheet.
...
1. **Header row** (space-between, top-aligned):
   - Left: 46px tile, radius 10px, background `--primary`, `--primary-foreground` initials "AHI" at 14px/700 (replaced by the uploaded logo when present); beside it the **trading name** 15px/700, and under it a 12px `--muted-foreground` block at line-height 1.6 with one line each for legal address, postal code, phone, email, website. **Every one of these lines reads from the Settings profile** — see State below.
   - Right: the word "Invoice" 26px/700 `-0.03em`, and under it the composed number `#AHI-EML-2026-014` in 12px mono muted.
...
6. **Payment details + signature** — grid `1fr 260px`, 30px gap, 28px top margin, bottom-aligned.
   - Left: `[payment_details]` label + a bordered block (1px `--border`, radius 8px, padding 12px, 12px text, line-height 1.7) — the **legal name** from the Settings profile in body font, then bank lines in mono muted (Wise/USD routing, account, SWIFT). The bank lines should also move into Settings when implemented.
...
- **Header row**: 36px tile, radius 8px, background `--accent`, `--primary` initials 12px/700; beside it client name 14px/700 `-0.01em`, and under it a mono row (11px, 6px gap): the **client code chip** — `padding: 1px 5px`, radius 4px, background `--muted`, text `--foreground` (e.g. `EML`) — followed by the country in `--muted-foreground`. The code is the segment used in invoice numbers and must be unique per user.
...
- **Company profile** (left) — Trading name, Legal name, Tax ID (mono), Address (2-row textarea), then a 2×2 grid (12px gap) of Postal code / Phone / Email / Website, then an 11px `--muted-foreground` note "These details print in the invoice header." Every field is bound two-way: editing it updates the invoice header live. Below, two side-by-side upload dropzones: 1px dashed `--border`, radius 8px, 76px tall, centered 16px icon (`image` / `pen-line`) over an 11px muted label — "Upload logo" and "Upload signature". Wire these to the existing base64 upload logic.
- **Numbering** (right, top) — Prefix (mono, e.g. `AHI` — user prefix only, no year), then a **pattern preview tile** (1px `--border`, radius 8px, padding `10px 12px`, background `--background`): the mono caption `[numbering.pattern]`, then `AHI-EML-2026-014` at 12px mono with the client-code segment in `--primary`, then an 11px `--muted-foreground` explainer at line-height 1.5 — "Your prefix, then the client code, then year and sequence. Client codes are set on each client." Below that: Next number (mono, e.g. `014`) and Default terms `<select>` (Net 15 / **Net 30** / Net 45 — Net 30 preselected; in React set it via `defaultValue`, not `selected` on the option). The due date defaults to issue date + terms.
...
- **Numbering**: changing the client in the editor's bill-to `<select>` rewrites both the address block and the invoice number. "New invoice" from a client card does the same.
...
| `seq` | string | zero-padded running sequence, e.g. `"014"` |
| `profile` | object | `tradeName, legalName, taxId, address, postal, phone, email, website` — printed in the invoice header and payment block |
...
2. **New `clients` table**: `id, user_id, code, name, country, address, created_at` — `code` is the 3-letter invoice-number segment, unique per `user_id`, RLS scoped to `user_id` exactly like `invoices`. Add `client_id` (nullable FK) to `invoices` so the dashboard and top-clients ranking can join rather than parsing `bill_to` text. Keep `bill_to` as the frozen snapshot printed on the invoice.
3. **New fields on `invoices`**: `reference` (replaces the `ship_to` use), and optionally `fx_rate` + `fx_currency` captured at issue time.
4. **Settings**: a `profile` table or a JSON blob for trading name, legal name, tax ID, address, postal code, phone, email, website, invoice prefix, next number, default terms, logo, signature (the last two already exist per-invoice as base64). The invoice header must render from this record, not from literals.

5. **Number composition**: `${prefix}-${client.code}-${year}-${seq}`. Regenerate on client change while the invoice is a draft; **freeze the number once the invoice leaves `draft`** (an issued invoice must never be renumbered). Duplicating an invoice takes the next sequence and resets status to `draft`.
